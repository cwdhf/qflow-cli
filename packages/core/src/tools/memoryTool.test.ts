  ...
  describe('shouldConfirmExecute', () => {
    ...
    it('should return confirmation details when memory file is not allowlisted', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join('~', QFLOW_DIR, 'QFLOW.md');
        expect(result.title).toBe(`Confirm Memory Save: ${expectedPath}`);
        expect(result.fileName).toContain(path.join('mock', 'home', QFLOW_DIR));
        expect(result.fileName).toContain('QFLOW.md');
        expect(result.fileDiff).toContain('Index: QFLOW.md');
