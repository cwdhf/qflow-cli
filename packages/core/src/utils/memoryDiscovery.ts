  ...
  for (const geminiMdFilename of geminiMdFilenames) {
    ...
    if (dir && folderTrust) {
      ...
      while (currentDir && currentDir !== path.dirname(currentDir)) {
        if (currentDir === path.join(resolvedHome, QFLOW_DIR)) {
          break;
        }
...
export async function loadGlobalMemory(
  ...
  const accessChecks = geminiMdFilenames.map(async (filename) => {
    const globalPath = path.join(userHome, QFLOW_DIR, filename);
    try {
...
/**
 * Traverses upward from startDir to stopDir, finding all QFLOW.md variants.
 *
...
async function findUpwardGeminiFiles(
  ...
  const geminiMdFilenames = getAllGeminiMdFilenames();
  const globalGeminiDir = path.join(homedir(), QFLOW_DIR);
...
/**
 * Loads hierarchical QFLOW.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  if (filePaths.length === 0) {
    if (debugMode)
      logger.debug('No QFLOW.md files found in hierarchy of the workspace.');
