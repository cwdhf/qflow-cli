...
const TEST_QFLOW_DIR = path.join(os.homedir(), QFLOW_DIR, TMP_DIR_NAME, hash);

const TEST_LOG_FILE_PATH = path.join(TEST_QFLOW_DIR, LOG_FILE_NAME);
...
async function cleanupLogAndCheckpointFiles() {
  try {
    await fs.rm(TEST_QFLOW_DIR, { recursive: true, force: true });
  } catch (_error) {
...
describe('Logger', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(TEST_QFLOW_DIR, { recursive: true });
  ...
  describe('initialize', () => {
    ...
    it('should handle invalid JSON in log file by backing it up and starting fresh', async () => {
      ...
      const logContent = await readLogFile();
      expect(logContent).toEqual([]);
      const dirContents = await fs.readdir(TEST_QFLOW_DIR);
      expect(
        dirContents.some(
          (f) =>
