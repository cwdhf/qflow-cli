...
The Qflow CLI includes built-in tools that the Qflow model uses to interact with
...
## Overview of Qflow CLI tools

In the context of the Qflow CLI, tools are specific functions or modules that
...
## How to use Qflow CLI tools

To use Qflow CLI tools, provide a prompt to the Qflow CLI. The process works as
follows:

1.  You provide a prompt to the Qflow CLI.
...
in mind. The Qflow CLI will typically:
...
- **Utilize sandboxing:** All tools are subject to restrictions enforced by
  sandboxing (see [Sandboxing in the Qflow CLI](../cli/sandbox.md)). This means
...
## Learn more about Qflow CLI's tools

Qflow CLI's built-in tools can be broadly categorized as follows:
