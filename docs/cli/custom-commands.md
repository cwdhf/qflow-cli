...
prompts as personal shortcuts within Qflow CLI. You can create commands that are
...
- `prompt` (String): The prompt that will be sent to the Qflow model when the
...
When a custom command attempts to execute a shell command, Qflow CLI will now
...
Qflow CLI will then execute the multi-line prompt defined in your TOML file.
