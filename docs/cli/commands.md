  ...
  - **Sub-commands:**
    - **`save`**
      ...
      - **Details on checkpoint location:** The default locations for saved chat
        checkpoints are:
        - Linux/macOS: `~/.qflow/tmp/<project_hash>/`
        - Windows: `C:\Users\<YourUsername>\.qflow\tmp\<project_hash>\`
  ...
  - **Sub-commands:**
    ...
    - **`show`**:
      - **Description:** Display the full, concatenated content of the current
        hierarchical memory that has been loaded from all `QFLOW.md` files. This
        lets you inspect the instructional context being provided to the Qflow
        model.
    - **`refresh`**:
      - **Description:** Reload the hierarchical instructional memory from all
        `QFLOW.md` files found in the configured locations (global,
        project/ancestors, and sub-directories). This command updates the model
        with the latest `QFLOW.md` content.
    - **`list`**:
      - **Description:** Lists the paths of the QFLOW.md files in use for
        hierarchical memory.
    - **Note:** For more details on how `QFLOW.md` files contribute to
...
- [**`/settings`**](./settings.md)
  ...
  - **Details:** This command provides a user-friendly interface for changing
    ...
    equivalent to manually editing the `.qflow/settings.json` file, but with
...
- **`/vim`**
  ...
  - **Features:**
    ...
    - **Persistent setting:** Vim mode preference is saved to
      `~/.qflow/settings.json` and restored between sessions
  - **Status indicator:** When enabled, shows `[NORMAL]` or `[INSERT]` in the
    footer

- **`/init`**
  - **Description:** To help users easily create a `QFLOW.md` file, this command
    analyzes the current directory and generates a tailored context file, making
    it simpler for them to provide project-specific instructions to the Qflow
    agent.
...
- **Environment variable:** When a command is executed via `!` or in shell mode,
  the `QFLOW_CLI=1` environment variable is set in the subprocess's environment.
