...
Qflow CLI supports a variety of themes to customize its color scheme and
appearance. You can change the theme to suit your preferences via the `/theme`
command or `"theme":` configuration setting.

## Available themes

Qflow CLI comes with a selection of pre-defined themes, which you can list using
the `/theme` command within Qflow CLI:
...
Selected themes are saved in Qflow CLI's
...
Qflow CLI allows you to create your own custom color themes by specifying them
...
**Security note:** For your safety, Qflow CLI will only load theme files that
...
- Select your custom theme using the `/theme` command in Qflow CLI. Your custom
