...
The Qflow CLI includes a Checkpointing feature that automatically saves a
...
1.  **A Git snapshot:** A commit is made in a special, shadow Git repository
    located in your home directory (`~/.qflow/history/<project_hash>`). This
...
`~/.qflow/tmp/<project_hash>/checkpoints`.
