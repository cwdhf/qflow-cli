...
which projects can use the full capabilities of the Qflow CLI. It prevents
...
Once the feature is enabled, the first time you run the Qflow CLI from a folder,
...
Your choice is saved in a central file (`~/.qflow/trustedFolders.json`), so you
will only be asked once per folder.

## Why trust matters: The impact of an untrusted workspace

When a folder is **untrusted**, the Qflow CLI runs in a restricted "safe mode"
...
1.  **Workspace settings are ignored**: The CLI will **not** load the
    `.qflow/settings.json` file from the project. This prevents the loading of
...
Granting trust to a folder unlocks the full functionality of the Qflow CLI for
that workspace.
...
- **View all trust rules**: To see a complete list of all your trusted and
  ...
  `~/.qflow/trustedFolders.json` file in your home directory.
...
2.  **Local trust file**: If the IDE is not connected, the CLI checks the
    central `~/.qflow/trustedFolders.json` file.
