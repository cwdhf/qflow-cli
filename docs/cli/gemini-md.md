# Provide context with QFLOW.md files

Context files, which use the default name `QFLOW.md`, are a powerful feature for
providing instructional context to the Qflow model. You can use these files to
...
2.  **Project root and ancestor context files:**
    - **Location:** The CLI searches for a `QFLOW.md` file in your current
...
### Example `QFLOW.md` file

Here is an example of what you can include in a `QFLOW.md` file at the root of a
TypeScript project:
...
You can break down large `QFLOW.md` files into smaller, more manageable
...
While `QFLOW.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "QFLOW.md"]
  }
}
```
