# Qflow CLI core: Tools API

The Qflow CLI core (`packages/core`) features a robust system for defining,
registering, and executing tools. These tools extend the capabilities of the
Qflow model, allowing it to interact with the local environment, fetch web
...
- **Tool (`tools.ts`):** An interface and base class (`BaseTool`) that defines
  ...
  - `parameterSchema`: A JSON schema defining the parameters that the tool
    accepts. This is crucial for the Qflow model to understand how to call the
    tool correctly.
...
- **Tool registry (`tool-registry.ts`):** A class (`ToolRegistry`) responsible
  ...
  - **Providing schemas:** Exposing the `FunctionDeclaration` schemas of all
    registered tools to the Qflow model, so it knows what tools are available
    and how to use them.
...
1.  **Model request:** The Qflow model, based on the user's prompt and the
...
8.  **Response to model:** The `llmContent` from the `ToolResult` is packaged as
    a `FunctionResponse` and sent back to the Qflow model so it can continue
...
- **Command-based discovery:** Advanced users or project administrators can
  ...
  by the Qflow CLI core, should output a JSON array of `FunctionDeclaration`
...
- **MCP server(s):** For more complex scenarios, one or more MCP servers can be
  ...
  Qflow CLI core can then discover and use tools exposed by these servers. As
