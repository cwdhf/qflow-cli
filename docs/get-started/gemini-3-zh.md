...
# Qflow CLI 上的 Gemini 3 Pro

我们很高兴地将 Gemini 3 Pro 带到 Qflow CLI。Gemini 3 Pro 目前在 Gemini
CLI 上对以下订阅用户可用：
...
- 使用**使用 Google 登录**选项进行身份验证。您将看到一个横幅，显示"Gemini
  3 现已可用"。如果您没有看到此横幅，请将您的 Qflow CLI 安装更新到最新版本。
- 填写此 Google 表单：[在 Qflow CLI 中访问 Gemini 3](https://goo.gle/geminicli-waitlist-signup)。提供您用于与 Gemini
...
## 如何在 Qflow CLI 中使用 Gemini 3 Pro

一旦您收到接受邮件——或者如果您是自动获得访问权限——您仍需要在 Gemini
CLI 中启用 Gemini 3 Pro。

要启用 Gemini 3 Pro，请在 Qflow CLI 中使用 `/settings`
命令，并将**预览功能**设置为 `true`。

更多信息，请参阅 [Qflow CLI 设置](../cli/settings.md)。
...
使用 Qflow CLI 时，您可能想要控制您的请求如何在模型之间路由。默认情况下，Gemini
CLI 使用**自动**路由。
...
[Qflow CLI 模型选择](../cli/model.md)。

## 如何在 Gemini Code Assist 上使用 Qflow CLI 启用 Gemini 3 Pro

如果您使用的是 Gemini Code Assist Standard 或 Gemini Code Assist
Enterprise，在 Qflow CLI 上启用 Gemini 3 Pro 需要配置您的发布渠道。使用 Gemini 3
...
- 导航到您与 Qflow CLI for Code Assist 一起使用的 Google Cloud 项目。
...
重新启动 Qflow CLI，您应该能够访问 Gemini 3 Pro。
