...
docker run --rm \
  -v "$QFLOW_PROJECT_DIR:/workspace:ro" \
...
const CACHE_FILE = '.qflow/hook-cache.json';
...
```bash
#!/usr/bin/env bash
LOG_FILE=".qflow/hooks/debug.log"
...
# Test the hook
cat test-input.json | .qflow/hooks/my-hook.sh
...
input=$(cat)
echo "$input" >> .qflow/hook-inputs.log
echo "Logged input"
```
...
.qflow/hook-cache.json
.qflow/hook-debug.log
.qflow/memory/session-*.jsonl
...
```bash
ls -la .qflow/hooks/my-hook.sh
chmod +x .qflow/hooks/my-hook.sh
```
...
echo "$QFLOW_PROJECT_DIR/.qflow/hooks/my-hook.sh"

# Verify file exists
test -f "$QFLOW_PROJECT_DIR/.qflow/hooks/my-hook.sh" && echo "File exists"
```
...
if [ -z "$QFLOW_PROJECT_DIR" ]; then
  echo "QFLOW_PROJECT_DIR not set" >&2
  exit 1
fi
...
env > .qflow/hook-env.log

# Check specific variables
echo "QFLOW_PROJECT_DIR: $QFLOW_PROJECT_DIR" >> .qflow/hook-env.log
echo "QFLOW_SESSION_ID: $QFLOW_SESSION_ID" >> .qflow/hook-env.log
echo "OPENAI_API_KEY: ${OPENAI_API_KEY:+<set>}" >> .qflow/hook-env.log
```
...
if [ -f "$QFLOW_PROJECT_DIR/.env" ]; then
  source "$QFLOW_PROJECT_DIR/.env"
fi
```
...
```bash
export QFLOW_TELEMETRY_LOG_PROMPTS=false
```
