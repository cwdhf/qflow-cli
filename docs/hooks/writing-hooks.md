# Writing hooks for Qflow CLI

This guide will walk you through creating hooks for Qflow CLI, from a simple
...
- Qflow CLI installed and configured
...
```bash
mkdir -p .qflow/hooks
cat > .qflow/hooks/log-tools.sh << 'EOF'
#!/usr/bin/env bash
...
# Log to file
echo "[$(date)] Tool executed: $tool_name" >> .qflow/tool-log.txt

# Return success (exit 0) - output goes to user in transcript mode
echo "Logged: $tool_name"
EOF

chmod +x .qflow/hooks/log-tools.sh
```

### Step 2: Configure the hook

Add the hook configuration to `.qflow/settings.json`:
        ...
        "hooks": [
          {
            "name": "tool-logger",
            "type": "command",
            "command": "$QFLOW_PROJECT_DIR/.qflow/hooks/log-tools.sh",
...
Run Qflow CLI and execute any command that uses tools:
...
Check `.qflow/tool-log.txt` to see the logged tool executions.
        ...
        "hooks": [
          {
            "name": "secret-scanner",
            "type": "command",
            "command": "$QFLOW_PROJECT_DIR/.qflow/hooks/block-secrets.sh",
        ...
        "hooks": [
          {
            "name": "auto-test",
            "type": "command",
            "command": "$QFLOW_PROJECT_DIR/.qflow/hooks/auto-test.sh",
        ...
        "hooks": [
          {
            "name": "git-context",
            "type": "command",
            "command": "$QFLOW_PROJECT_DIR/.qflow/hooks/inject-context.sh",
...
mkdir -p .qflow/hooks .qflow/memory
        ...
        "hooks": [
          {
            "name": "init-assistant",
            "type": "command",
            "command": "node $QFLOW_PROJECT_DIR/.qflow/hooks/init.js",
        ...
        "hooks": [
          {
            ...
            "type": "command",
            "command": "node $QFLOW_PROJECT_DIR/.qflow/hooks/inject-memories.js",
        ...
        "hooks": [
          {
            "name": "rag-filter",
            "type": "command",
            "command": "node $QFLOW_PROJECT_DIR/.qflow/hooks/rag-filter.js",
        ...
        "hooks": [
          {
            "name": "security-check",
            "type": "command",
            "command": "node $QFLOW_PROJECT_DIR/.qflow/hooks/security.js",
            "description": "Prevent committing secrets"
          }
        ]
      }
        ...
        "hooks": [
          {
            "name": "auto-test",
            "type": "command",
            "command": "node $QFLOW_PROJECT_DIR/.qflow/hooks/auto-test.js",
            "description": "Run tests after code changes"
          }
        ]
      }
        ...
        "hooks": [
          {
            ...
            "type": "command",
            "command": "node $QFLOW_PROJECT_DIR/.qflow/hooks/record.js",
        ...
        "hooks": [
          {
            ...
            "type": "command",
            "command": "node $QFLOW_PROJECT_DIR/.qflow/hooks/consolidate.js",
...
async function main() {
  const projectDir = process.env.QFLOW_PROJECT_DIR;
...
**`.qflow/hooks/inject-memories.js`:**
...
async function main() {
  ...
  const projectDir = process.env.QFLOW_PROJECT_DIR;
  const sessionId = process.env.QFLOW_SESSION_ID;
