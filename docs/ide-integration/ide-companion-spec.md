# Qflow CLI companion plugin: Interface specification

> Last Updated: September 15, 2025

This document defines the contract for building a companion plugin to enable
Qflow CLI's IDE mode. For VS Code, these features (native diffing, context
...
Qflow CLI and the IDE plugin communicate through a local communication channel.
...
For Qflow CLI to connect, it needs to discover which IDE instance it's running
...
- **Tie-breaking with environment variables (recommended):** For the most
  ...
  set the `QFLOW_CLI_IDE_SERVER_PORT` environment variable in the integrated
  ...
  `QFLOW_CLI_IDE_SERVER_PORT` variable to identify and connect to the correct
  window's server.
