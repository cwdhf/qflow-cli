...
Qflow CLI can integrate with your IDE to provide a more seamless and
...
- **VS Code commands:** You can access Qflow CLI features directly from the VS
  Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `Qflow CLI: Run`: Starts a new Qflow CLI session in the integrated terminal.
  - `Qflow CLI: Accept Diff`: Accepts the changes in the active diff editor.
  - `Qflow CLI: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `Qflow CLI: View Third-Party Notices`: Displays the third-party notices for
    the extension.
...
When you run Qflow CLI inside a supported editor, it will automatically detect
...
manually, you can run the following command inside Qflow CLI:
...
When enabled, Qflow CLI will automatically attempt to connect to the IDE
companion extension.
...
- Open the Command Palette and run **Qflow CLI: Accept Diff**.
...
- Open the Command Palette and run **Qflow CLI: Close Diff Editor**.
...
If you are using Qflow CLI within a sandbox, please be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker container:** If you run Qflow CLI inside a Docker (or Podman)
...
- **Message:**
  `🔴 Disconnected: Directory mismatch. Qflow CLI is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  `IDE integration is not supported in your current environment. To use this feature, run Qflow CLI in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running Qflow CLI in a terminal or environment that is
    not a supported IDE.
  - **Solution:** Run Qflow CLI from the integrated terminal of a supported IDE,
