...
problems encountered while using Qflow CLI.
...
your total token usage using the `/stats` command in Qflow CLI.

## Installation and updates

### How do I update Qflow CLI to the latest version?
...
- **Use a `.env` file:** Create a `.env` file in your project's `.gemini`
  directory (`.qflow/.env`) and store your keys there. Qflow CLI will
...
### Where are the Qflow CLI configuration and settings files stored?

The Qflow CLI configuration is stored in two `settings.json` files:

1.  In your home directory: `~/.qflow/settings.json`.
2.  In your project's root directory: `./.qflow/settings.json`.

Refer to [Qflow CLI Configuration](./get-started/configuration.md) for more
details.
...
limits to Gemini Code Assist and Qflow CLI. These are shared across Qflow CLI
...
### What is the privacy policy for using Gemini Code Assist or Qflow CLI if I've subscribed to Google AI Pro or Ultra?
...
across both Gemini 2.5 Pro and Flash. They are shared quota across Qflow CLI and
agent mode in Gemini Code Assist IDE extensions. You can learn more about quota
limits for Qflow CLI, Gemini Code Assist and agent mode in Gemini Code Assist at
[Quotas and limits](https://developers.google.com/gemini-code-assist/resources/quotas).

### If I upgrade to higher limits for Qflow CLI and Gemini Code Assist by purchasing a Google AI Pro or Ultra subscription, will Gemini start using my data to improve its machine learning models?
...
Search the
[Qflow CLI Q&A discussions on GitHub](https://github.com/google-gemini/gemini-cli/discussions/categories/q-a)
or
